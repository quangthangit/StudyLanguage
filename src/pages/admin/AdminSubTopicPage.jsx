...
import { createSubTopic, deleteSubTopic, getSubTopics } from "../../services/subtopicService";

export default function AdminSubTopicPage() {
    ...
    const handleAddType = async () => {
        ...
        if (newDetailsubtype.name.english && newDetailsubtype.name.korean && newDetailsubtype.name.vietnamese) {
            setLoading(true);
            try {
                const response = await createSubTopic(newDetailsubtype);
                if (response?.status === 201) {
                    console.log(response.data)
                    setDetailsubtype((prev) => [...prev, response.data.subtopic]);
                    setNewDetailsubtype( { 
                        topicId : "",
                        name: { english: "", korean: "", vietnamese: "" } 
                    });
                ...
                <div className="bg-white p-6 rounded shadow">
                    ...
                    <form className="space-y-4">
                        ...
                        <div>
                            ...
                            <select
                                value={newDetailsubtype.topicId}
                                onChange={(e) => setNewDetailsubtype({ ...newDetailsubtype, topicId: e.target.value })}
